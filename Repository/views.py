...
def database_query_task_list(request):  # 展示项目的任务列表
    if request.method == 'POST':
        ...
        records = Task.objects.filter(repo_id=repo_id).values('task_info','status','deadline')  # 查询Task表找出所有任务信息，包括任务状态，截至日期等等
...
def database_project_insert_one(request):  # 用户选择一个项目，把该项目放入数据库，并将当前用户设为超级管理员
    if request.method == 'POST':
        ...
        HttpResponse(new_record_repo.id)
        records = Repository.objects.filter(url=url,repo_name=repo_name)
